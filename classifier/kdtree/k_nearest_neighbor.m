...
hold off;
fprintf( 'The training error is:\n' );
fprintf( '%f\n', error_nu );
fprintf( 'The testing error is:\n');
fprintf( '%f\n', error_pi );
fprintf( '--------------------\n');
...
ylabel('\nu(A_K, 10)');
mkdir( 'outputs/KNN' );
saveas(gcf, 'outputs/KNN/3.4(2).png', 'png');
fprintf( 'The cross-validation error is:\n');
fprintf( '%f\n', error_c );
return;
...
for n = 1: len
    ...
    %end
    q_signs = train_set( idxs, 3 );
    q_sign = sign( sum ( q_signs ) );
