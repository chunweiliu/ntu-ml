...
T = 100;
g = plr( x, y, T );
%copyfile('demo_plr_linux.gif', '/home/master/97/r97032/htdocs/show/plr/demo_plr_linux.gif ');
%delete('demo_plr_linux.gif');
return;
...
for t = 2:T
    ...
    [xx,Map] = draw(W, x, y, t);
    imwrite(xx,Map,'demo_plr_linux.gif','GIF','WriteMode','append','DelayTime',time);
end
g = W / norm(W);
return;
...
for k = 1:size(x,1)
    s = (y(k)+3)/2;
    if t == k
        plot( x(k, 1), x(k, 2), m(s), 'Color','red' );
    else
