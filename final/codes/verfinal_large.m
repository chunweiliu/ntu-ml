function verErrorRs = verfinal_large()
fold = 5;
fid = fopen('outputs/ver_large.txt', 'w');
%step1: read data
file_data = load('dataset/proj_large_train.dat');
...
for n = 1:length(NodeNums)
    ...
    fprintf(fid, 'Training Data = 40000, Argument (T = 100, TreeNode = %d); The %d fold corss validation error EV = %f\n', NodeNum, fold, verErrorRs(n));
end
fclose(fid);
