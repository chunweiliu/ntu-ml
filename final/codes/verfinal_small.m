function verErrorRs = verfinal_small()
fold = 5;
fid = fopen('outputs/ver_small.txt', 'w');
%step1: read data
file_data = load('dataset/proj_small_train.dat');
...
for n = 1:length(NodeNums)
    ...
    fprintf(fid, 'Training Data = 400, Argument (T = 100, TreeNode = %d); The %d fold corss validation error EV = %f\n', NodeNum, fold, verErrorRs(n));
end
fclose(fid);
